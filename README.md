...
- 诊断页提供一键“自动校准准确率”：后端直接读取该诊断已保存的多轮文本与能力证据，自动完成逐能力项一致性复核并回写准确率，无需学习者填写 16 项表单
...
- **精确上下文范围**：追问模型只能收到岗位名称、岗位技能、结构化摘要、最近 6 条消息和当前输入；正式诊断只接收最多 8 条学习者消息与 1 份不超过 1600 字的已确认摘要。它不会读取无关历史、其他 Agent 的内部推理或整库原文。
...
诊断页的 `POST /api/assessment/{id}/auto-calibrate` 是面向学习者的一键自动证据复核：按钮点击后无需继续输入，系统根据已持久化的多轮对话、正负能力陈述、行动词和可验证结果信号计算“自动证据校准准确率”。该指标会标记 `mode=automatic_evidence_review` 与 `is_ground_truth=false`，用于检查诊断和现有证据是否一致，不冒充比赛测试集上的真实准确率。比赛验收使用 `POST /api/assessment/{id}/calibrate`，以客观题、实操测试或专家标注为可信真值计算准确率与 MAE。
