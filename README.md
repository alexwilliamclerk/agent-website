...
诊断页的 `POST /api/assessment/{id}/auto-calibrate` 是面向学习者的一键自动证据复核：按钮点击后无需继续输入，系统只评价本轮明确提及的能力证据。能力连续分数采用 `±5 分` 容差带，之后按 `自动证据校准得分 = 70% × [1-max(0, MAE-0.05)] + 25% × 诊断置信度 + 5% × 显式证据覆盖率` 计算；旧版二元正确率保存在 `binary_item_accuracy` 中供审计。该指标会标记 `mode=automatic_evidence_review` 与 `is_ground_truth=false`，用于检查诊断和现有证据是否一致，不冒充比赛测试集上的真实准确率。比赛验收使用 `POST /api/assessment/{id}/calibrate`，以客观题、实操测试或专家标注为可信真值计算准确率与 MAE。
